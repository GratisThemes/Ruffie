...
<section id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
  ...
  <article class="entry-content">
    ...
    <?php 
    $ruffie_anchor_content = apply_filters( 'the_content', get_the_content() );
    $ruffie_anchor = false;

    // Only get video from the content if a playlist isn't present.
    $anchor = get_media_embedded_in_content( $ruffie_anchor_content, array( 'a' ) );
    
    if ( !empty( $ruffie_anchor ) ) {

      foreach ( $ruffie_anchor as $ruffie_anchor_html ) {
        echo $ruffie_anchor_html;
      }
